module.exports.digest = async (data) => {
  try {
    ...
    const countChildFriendly = data.reduce(
      (acc, e) =>
        acc.set(e.child_friendly, (acc.get(e.child_friendly) || 0) + 1),
      new Map()
    );
    ...
    const strangerFriendlyTopWeighting = Math.max(
      ...strangerFriendlyMap.keys()
    );
    ...
    console.log('error loadind the data');
  }
};
